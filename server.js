...
const server = net.createServer((socket) => {
  ...
  socket.on("data", (data) => {
    ...
    if (!clientName) {
      ...
      privileges[clientName] =
        clientName === "admin"
          ? "privilegje te plota"
          : "privilegje te pjesshme"; // Admin ose lexues
  ...
  - delete <emriSkedarit>:Fshin një skedar
    `);
      ...
      if (command[0] === "servermessage" && command.length > 1) {
        const serverMessage = message.slice(13);
        console.log(`Klienti ${clientName}:${serverMessage}`);
        socket.write(`Mesazh për serverin: ${serverMessage}\n`);
        return;
      }
      if (privileges[clientName] === "privilegje te pjesshme") {
        // Komandat që nuk lejohen për klientët me privilegje të pjesshme
        if (
          command[0] === "write" ||
          command[0] === "execute" ||
          command[0] === "delete" ||
          ...
          socket.write(
            `Për klientët me privilegje të pjesshme, nuk lejohet ekzekutimi i komandës '${command[0]}'.\n`
          );
          return;
        }
      }
      ...
      if (command[0] === "chat" && command.length > 2) {
        ...
        if (clients[targetClient]) {
          ...
          socket.write(
            `Mesazhi dërguar për klientin ${targetClient}: ${chatMessage}\n`
          );
        } else {
      ...
      // Komanda broadcast <message> (për dërgimin e mesazheve për të gjithë klientët)
        ...
        console.log(
          `Mesazhi i dërguar nga ${clientName} për të gjithë: ${broadcastMessageText}`
        );
      }
      // Komanda write <file> <content> (për shkruarje në skedar)
      else if (
        command[0] === "write" &&
        privileges[clientName] === "privilegje te plota"
      ) {
        if (command.length < 3) {
          socket.write(
            "Specifikoni emrin e skedarit dhe përmbajtjen për shkruarje.\n"
          );
          return;
        }
        ...
        console.log(
          `Privilegji i klientit ${clientName}: ${privileges[clientName]}`
        );
        console.log(
          `Po shkruajmë në skedarin: ${filePath} me përmbajtje: ${content}`
        );
      ...
      // Komanda read <file> (për të lexuar një skedar)
        ...
        command[0] === "read" &&
        (privileges[clientName] === "privilegje te plota" ||
          privileges[clientName] === "privilegje te pjesshme")
      ) {
        if (!command[1]) {
      ...
      // Komanda list <folder> (për të listuar skedarët në një folder)
        ...
        fs.readdir(folderPathToList, (err, files) => {
          if (err) {
            ...
            socket.write(
              `Përmbajtja e folderit ${folderPathToList}: \n${files.join(
                "\n"
              )}\n`
            );
          }
      ...
      // Komanda create <file> <content> (për të krijuar një skedar të ri)
        ...
        if (command.length < 3) {
          socket.write(
            "Specifikoni emrin e skedarit dhe përmbajtjen për krijim.\n"
          );
          return;
        }
        const filePath = path.join(folderPath, command[1]);
...
function broadcastMessage(message, senderName) {
  for (const clientName in clients) {
    const clientSocket = clients[clientName];
    clientSocket.write(
      `Mesazh i dërguar nga ${senderName} për të gjithë: ${message}\n`
    );
  }
}
...
server.listen(3000, "127.0.0.1", () => {
  ...
  console.log(
    "- chat <clientName> <mesazh> : Dërgon mesazh privat për një klient të caktuar."
  );
  console.log(
    "- broadcast <mesazh>     : Dërgon mesazhin për të gjithë klientët."
  );
